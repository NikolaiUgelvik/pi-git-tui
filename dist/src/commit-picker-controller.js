...
export class CommitPickerController {
    ...
    renderOverlayLines(baseLineCount, width, theme) {
        ...
        return renderSearchOverlayFrame(frame, theme, "Select commit", "type search • backspace edit • ↑↓ navigate • enter select • ? help • esc cancel", this.renderSearchLine(theme), this.renderBodyRows(frame.maxItems, theme));
    }
    renderSearchLine(theme) {
        const query = this.list.searchQuery.length > 0 ? `${this.list.searchQuery}▌` : theme.fg("muted", "type to filter commits");
