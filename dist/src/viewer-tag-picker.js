...
export class DiffViewerTagPicker extends DiffViewerWorktreePicker {
    ...
    constructor(...args) {
        ...
        this.tagPickerController = new TagPickerController({
            onSelect: (tag) => void this.viewTag(tag).catch((error) => this.showAsyncError(error)),
            onRequestTargets: () => void this.loadTagTargets().catch((error) => this.showAsyncError(error)),
            onCreate: (creation) => void this.runTagCreate(creation).catch((error) => this.showAsyncError(error)),
    ...
    async loadTagList() {
        ...
        const outcome = await this.runLoad({
            ...
            apply: (tags) => {
                if (this.tagPickerController.isCurrent(request))
    ...
    tagListRefreshIntent(cwd, expectedTagName, request) {
        return {
            ...
            apply: (tags) => {
                ...
                if (tags.some((tag) => tag.name === expectedTagName))
                    this.tagPickerController.showTagList();
            },
        };
    }
