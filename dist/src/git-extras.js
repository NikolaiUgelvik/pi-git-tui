...
import { assertGitSuccess, ensureGitRepository, GitExitError, isUnbornHeadResult, probeGit, requireGitRepository, runGit, } from "./git-service.js";
...
function indexEntryPaths(output) {
    return new Set(output.split("\0").flatMap((entry) => {
        const separator = entry.indexOf("\t");
        return separator < 0 ? [] : [entry.slice(separator + 1)];
    }));
}
async function classifyDiscardPaths(pi, root, paths, signal) {
    const indexResult = await runGit(pi, root, withLiteralPaths(["-c", "core.quotepath=false", "ls-files", "--stage", "-z"], paths), { signal });
    const untrackedResult = await runGit(pi, root, withLiteralPaths(["-c", "core.quotepath=false", "ls-files", "--others", "--exclude-standard", "-z"], paths), { signal });
    const ignoredResult = await runGit(pi, root, withLiteralPaths(["-c", "core.quotepath=false", "ls-files", "--others", "--ignored", "--exclude-standard", "-z"], paths), { signal });
    const head = new Set();
    if (await hasHead(pi, root, signal)) {
        const headResult = await runGit(pi, root, withLiteralPaths(["-c", "core.quotepath=false", "ls-tree", "-r", "--name-only", "-z", "HEAD"], paths), { signal });
...
async function restoreHeadPaths(pi, root, paths, signal) {
    if (paths.length === 0)
        return;
...
async function removeIndexOnlyPaths(pi, root, paths, signal) {
    if (paths.length === 0)
        return;
...
async function cleanUntrackedPaths(pi, root, paths, signal) {
    if (paths.length === 0)
        return;
...
function assertNoDiff(result, args, root, kind) {
    if (result.code === 0)
        return;
    if (result.code === 1) {
        throw new GitExitError({ ...result, stderr: result.stderr || `${kind} changes remain after discard` }, args, undefined, root);
    }
    assertGitSuccess(result, args, [0, 1], root);
}
async function verifyDiscard(pi, root, aliases, cleanedPaths, signal) {
...
export async function discardFileChanges(pi, cwd, file, signal) {
    ...
    for (const path of aliases) {
        if (!classification.head.has(path) &&
