...
function tokenKind(grapheme) {
    if (WHITESPACE_GRAPHEME.test(grapheme))
        return "whitespace";
    return WORD_GRAPHEME.test(grapheme) ? "word" : "symbol";
}
export function tokenizeIntralineLine(text, maximumGraphemes, maximumTokens) {
    const tokens = [];
    let graphemeCount = 0;
    let activeKind;
...
function linePairScore(left, right) {
    return 2 * diceSimilarity(left, right) - 1_000;
}
export function alignIntralineLines(oldLines, newLines) {
    ...
    for (let oldIndex = 1; oldIndex <= oldLines.length; oldIndex++) {
        for (let newIndex = 1; newIndex <= newLines.length; newIndex++) {
            ...
            let best = scores[(oldIndex - 1) * columns + newIndex - 1] +
                linePairScore(oldLines[oldIndex - 1], newLines[newIndex - 1]);
            let direction = 0;
...
export function changedTokenIndices(oldTokens, newTokens) {
    ...
    while (oldIndex > 0 && newIndex > 0) {
        ...
        else if ((lengths[(oldIndex - 1) * columns + newIndex] ?? 0) >= (lengths[oldIndex * columns + newIndex - 1] ?? 0)) {
            oldIndex--;
        }
        else {
            newIndex--;
