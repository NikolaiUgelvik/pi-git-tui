...
function indexSourceBytes(group, headSizes, indexSizes) {
    if (group.paths.some((path) => indexSizes.changedPaths.has(path))) {
        return { totalBytes: 0, maxFileBytes: 0, changed: true };
    }
    return measurementFromSizes(group.paths.map((path) => Math.max(headSizes.get(path) ?? 0, indexSizes.sizes.get(path) ?? 0)));
}
...
function sourceBytes(group, headSizes, states, indexSizes, scope) {
    if (scope === "staged" && indexSizes)
        return indexSourceBytes(group, headSizes, indexSizes);
    if (scope === "working" && indexSizes) {
        if (group.paths.some((path) => indexSizes.changedPaths.has(path))) {
            return { totalBytes: 0, maxFileBytes: 0, changed: true };
        }
        return worktreeSourceBytes(group, indexSizes.sizes, states);
    }
...
export function selectTrackedGroups(groups, snapshot, headSizes, states, indexSizes, scope, budget, omissions, signal) {
    const selected = [];
    let selectedBytes = 0;
    let stopped;
    for (const group of groups) {
        throwIfGitAborted(signal);
        const source = sourceBytes(group, headSizes, states, indexSizes, scope);
        if (source.changed) {
