...
export class DiffViewerFrame extends DiffViewerCore {
    diffMaximumColumn = 0;
    renderCache = new ViewerRenderCache([]);
    renderCacheStats() {
    ...
    render(width) {
        ...
        const frame = (content) => fit(`${side}${fit(content, geometry.innerWidth)}${side}`, geometry.width);
    ...
    renderMainPanel(geometry) {
        ...
        return Array.from({ length: geometry.panelRows }, (_, index) => `${treeLines[index] ?? " ".repeat(geometry.treeWidth)}${separator}${diffLines[index] ?? " ".repeat(geometry.diffWidth)}`);
    }
    renderSummaryTitle(width) {
        return fit(this.theme.fg("accent", this.theme.bold("▶ Summary")), width);
    }
    renderTooSmallFrame(geometry) {
    ...
    renderPanelTitle(panel, width, single = false) {
    ...
    renderOperationFooter(width) {
        const operation = this.operationSnapshot();
        switch (operation.state) {
            case "running":
                return this.theme.fg("dim", prioritizedFooter(`… ${this.footerSummary(operation.summary, "Working…")}`, ["Esc cancel", "? help"], width));
            case "cancelling":
                return this.theme.fg("warning", prioritizedFooter(`… ${this.footerSummary(operation.summary, "Cancelling…")}`, ["? help"], width));
            case "reconciling":
                return this.theme.fg("warning", prioritizedFooter(`… ${this.footerSummary(operation.summary, "Reconciling…")}`, ["? help"], width));
            case "refreshFailed":
    ...
    footerSummary(summary, fallback) {
        return summary ?? fallback;
    }
    renderRefreshFailureFooter(summary, successMessage, width) {
        const success = successMessage ? `✓ ${successMessage} • ` : "";
        return this.theme.fg("warning", prioritizedFooter(`${success}⚠ ${summary ?? "Diff refresh failed"}`, ["r retry refresh", "? help", "q close"], width));
    }
    renderOperationFailureFooter(summary, width) {
    ...
    renderDocumentFooter(width) {
        const operation = this.operationSnapshot();
        if (this.documentState.failure) {
            return this.theme.fg("warning", prioritizedFooter(`⚠ ${this.documentState.failure.summary}`, ["r retry", "? help", "q close"], width));
        }
        if (this.documentState.failedTarget) {
            return this.theme.fg("warning", prioritizedFooter(`⚠ ${this.documentState.failedTarget.summary}`, ["r retry target", "W working tree", "? help", "q close"], width));
        }
        if (this.error) {
        ...
        if (operation.state === "succeeded" && operation.successMessage) {
            return this.theme.fg("success", prioritizedFooter(`✓ ${operation.successMessage}`, ["r reload", "? help", "q close"], width));
        }
        if (this.statusMessage) {
            return this.theme.fg("success", prioritizedFooter(`✓ ${this.statusMessage}`, ["r reload", "? help", "q close"], width));
        }
    ...
    renderTree(width, height) {
        ...
        const selectedRow = Math.max(0, rows.findIndex((row) => row.fileIndex === this.selectedFileIndex));
    ...
    colorTreeFile(line, file, selected) {
