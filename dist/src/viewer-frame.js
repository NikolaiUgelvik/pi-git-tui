...
function diffDisplayRowColor(row) {
    switch (row.type) {
        case "addition":
...
export class DiffViewerFrame extends DiffViewerNavigation {
    render(width) {
        ...
        const frame = (content) => fit(`${side}${fit(content, innerWidth)}${side}`, width);
    ...
    renderFooter(width) {
        ...
        const enterAction = this.focusedPanel === "tree" ? " • Enter stage/unstage • Shift+Enter stage/unstage all • D discard" : "";
        return fit(this.theme.fg("dim", `${omissionNotice}focus:${focusLabel} • tab switch • n/p files • ${arrows}${enterAction} • PgUp/PgDn scroll • Home/End jump • c commits • C commit • b branches • w worktrees • s stash • ^P commands • ? help • q close`), width);
    }
