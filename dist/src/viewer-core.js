...
export class DiffViewerCore extends DiffViewerNavigationBase {
    ...
    helpOverlayState = new HelpOverlayState();
    featureOverlays = new ViewerOverlayCoordinator();
    pickerState = "closed";
    get helpContext() {
        return this.helpOverlayState.context;
    }
    handleHelpInput(data, allowPrintableShortcut = true) {
    ...
    handleOperationCancelInput(data) {
    ...
    handleReturnToWorkingTreeInput(data) {
        if (data !== "W") {
            return false;
        }
        ...
        if (this.requireViewerAction("workingTree") && this.canStartForegroundOperation("returning to the working tree")) {
            this.returnToWorkingTree().catch((error) => this.showAsyncError(error));
        }
        return true;
    }
