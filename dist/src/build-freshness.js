...
function collectInputPaths(root) {
    const fixedFiles = BUILD_INPUT_FILES.filter((path) => existsSync(resolve(root, path)));
    const sourceFiles = BUILD_INPUT_DIRECTORIES.flatMap(({ directory, suffix }) => collectFiles(root, directory, (path) => path.endsWith(suffix)));
...
function parseManifest(root) {
    const value = JSON.parse(readFileSync(resolve(root, BUILD_MANIFEST_PATH), "utf8"));
    if (!value || typeof value !== "object")
        throw new Error("build manifest is not an object");
...
function requireRuntimeRecord(record, kind) {
    if (typeof record?.path !== "string")
        throw new Error(`build manifest has an invalid ${kind} path`);
    if (typeof record.bytes !== "number")
        throw new Error(`build manifest has invalid ${kind} bytes`);
    if (typeof record.sha256 !== "string")
        throw new Error(`build manifest has an invalid ${kind} hash`);
    return record;
}
function verifyManifestFiles(root, kind, actualPaths, candidates) {
    ...
    for (const [index, record] of records.entries()) {
        ...
        if (path !== record.path)
            throw new Error(`${kind} file list changed near ${path ?? record.path}`);
...
function packageRootForCompiledEntry(entryUrl) {
    const entryPath = fileURLToPath(entryUrl);
    const extensionDirectory = dirname(entryPath);
    const distDirectory = dirname(extensionDirectory);
    if (basename(extensionDirectory) !== "extensions" || basename(distDirectory) !== "dist")
        return undefined;
    return dirname(distDirectory);
}
function hasSourceCheckout(root) {
    return existsSync(resolve(root, "src")) || existsSync(resolve(root, "extensions/diff.ts"));
}
function lockedCompiler(root) {
    const lock = JSON.parse(readFileSync(resolve(root, "package-lock.json"), "utf8"));
    const version = lock.packages?.["node_modules/typescript"]
        ?.version;
    if (typeof version !== "string" || !version)
        throw new Error("package-lock.json does not pin TypeScript");
    return `typescript@${version}`;
}
function verifyCompiledBuild(root) {
...
export function assertCompiledBuildIsConsistent(entryUrl) {
