...
export async function* readNulRecords(outputPath, signal) {
...
function splitCompleteLines(buffered) {
    const lines = [];
    let offset = 0;
...
export async function* readPatchChunks(outputPath, signal) {
    const decoder = new StringDecoder("utf8");
    let buffered = "";
    let current = "";
    for await (const bytes of createReadStream(outputPath)) {
...
//# sourceMappingURL=git-output-file.js.map