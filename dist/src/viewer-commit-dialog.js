...
export class DiffViewerCommitDialog extends DiffViewerCommitPicker {
    commitDialogEpoch = 0;
    commitMessageField = new SingleLineTextField("", "commit message");
    get commitMessage() {
    ...
    requestGeneratedCommitMessage(signal) {
    ...
    commitDialogOverlayLines(frame) {
        ...
        return renderOverlayFrame(frame, ` ${this.theme.fg("accent", this.theme.bold(this.commitDialogTitle()))}`, ` ${this.theme.fg("dim", hint)}`, this.commitDialogBodyRows(frame.innerWidth));
    }
    commitDialogTitle() {
        if (!this.commitAmend) {
    ...
    commitDialogBodyRows(innerWidth) {
        ...
        const stats = this.document.mode === "working" ? this.document.staged.stats : { files: 0, additions: 0, deletions: 0 };
    ...
    renderCommitMessageInput(width) {
        return this.commitMessageField.render(width, this.commitMessageField.focused, this.theme.fg("muted", "commit message"));
    }
}
