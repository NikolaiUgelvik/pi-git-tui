import { commitOmission, loadBoundedStagedEntries, StagedRawEntryDecoder, } from "./commit-staged-snapshot.js";
...
async function resolveHistoricalBase(pi, root, revision, signal) {
    ...
    if (values[0] !== commitOid)
        throw new Error("Git returned parents for a different commit");
...
async function loadHistoricalEntries(pi, root, base, maxEntries, signal) {
    return withGitOutputFile(pi, root, (outputPath) => rawDiffArgs(base, outputPath), async (outputPath) => {
        ...
        for await (const record of readNulRecords(outputPath, signal)) {
            ...
            if (!entry)
                continue;
            if (entries.length < Math.max(0, maxEntries))
                entries.push(entry);
            else
                skippedEntries++;
        }
...
function omittedHistoricalFile(entry, omission) {
    return {
        path: entry.path,
...
function entryIndexByPath(entries) {
    return new Map(entries.flatMap((entry) => entry.paths.map((path) => [path, entry.index])));
}
function historicalPatchChunk(raw, indexes) {
...
function recordCaptureOmission(omissions, entries, chunk, index, reason, details) {
...
function retentionOmission(state, chunk, budget) {
...
function retainHistoricalChunk(state, chunk, entries, omissions, budget) {
    const sequence = state.sequence++;
    if (chunk.entryIndex !== undefined)
        state.captured.add(chunk.entryIndex);
    if (chunk.entryIndex !== undefined && omissions.has(chunk.entryIndex))
        return;
...
function completeSelectedEntries(selected, captured, omissions) {
    for (const entry of selected) {
        if (captured.has(entry.index) || omissions.has(entry.index))
            continue;
        omissions.set(entry.index, commitOmission(entry.index, entry.path, "unsupported-file", {
            detail: "Git produced no patch for this historical entry.",
        }));
    }
}
function unenumeratedHistoricalFiles(count, limitFiles) {
    if (count === 0)
        return;
...
export async function captureHistoricalDiff(pi, root, revision, budget = DEFAULT_TRACKED_DIFF_BUDGET, signal) {
    ...
    const pathGroups = chunkLiteralPathGroups(selected.map((entry) => ({ value: entry, paths: entry.paths })), budget, patchDiffArgs(base, []));
    ...
    for (const batch of pathGroups.batches) {
        ...
        await withGitOutputFile(pi, root, (outputPath) => patchDiffArgs(base, paths, outputPath), async (outputPath) => {
    ...
    if (aggregateOmission)
        omittedFiles.push(aggregateOmission);
    return {
...
//# sourceMappingURL=git-historical-diff-capture.js.map