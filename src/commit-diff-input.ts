...
function commitPatchChunks(raw: string, entries: readonly StagedEntry[]): CommitPatchChunk[] {
  const entryByPath = new Map(entries.flatMap((entry) => entry.paths.map((path) => [path, entry.index] as const)))
...
function retainCommitPatch(
  chunks: readonly CommitPatchChunk[],
  entries: readonly StagedEntry[],
  omissions: Map<number, CommitOmission>,
  budget: CommitPromptBudget,
): { patch: string; includedFiles: number } {
  const totals = new Map<number, { chars: number; lines: number }>()
  ...
  let retainedChars = 0
  let retainedLines = 0
  let stopped: "chars" | "lines" | undefined
  for (const [index, chunk] of chunks.entries()) {
    const key = chunk.entryIndex ?? -(index + 1)
    if (retained.has(key) || (key >= 0 && omissions.has(key))) continue
    ...
    if (stopped === "chars") {
      omissions.set(
        key,
        commitOmission(omissionIndex, path, "capture-overflow", {
    ...
    if (stopped === "lines") {
      omissions.set(
        key,
        commitOmission(omissionIndex, path, "aggregate-line-budget", { limitLines: budget.maxPatchLines }),
      )
      continue
    }
    ...
    if (retainedLines + total.lines > budget.maxPatchLines) {
      ...
      omissions.set(
        key,
        commitOmission(omissionIndex, path, "aggregate-line-budget", {
...
function wholeLinesWithin(text: string, maxChars: number): string {
...
function omissionLines(omissions: readonly CommitOmission[], maxChars: number): string {
  if (omissions.length === 0 || maxChars <= 0) return ""
  ...
  for (const item of omissions) {
    ...
    if (chars + 1 + line.length > maxChars) break
    lines.push(line)
    chars += 1 + line.length
    shown++
  }
  if (shown < omissions.length) {
    ...
    if (chars + 1 + remaining.length <= maxChars) lines.push(remaining)
  }
  return lines.join("\n")
}

function joinInputSections(sections: readonly string[]): string {
  return sections.filter(Boolean).join("\n\n")
}

function buildCommitInputText(
  stat: string,
  patch: string,
  includedFiles: number,
  omissions: readonly CommitOmission[],
  budget: CommitPromptBudget,
): string {
  ...
  const statBudget = Math.max(
    0,
    Math.min(budget.maxStatChars, budget.maxInputChars - header.length - reservedPatch - usedByDetails - 2),
  )
...
interface CapturedCommitPatch {
  readonly stat: string
  readonly patch: string
  readonly includedFiles: number
}

const EMPTY_COMMIT_PATCH: CapturedCommitPatch = { stat: "", patch: "", includedFiles: 0 }

function markCommitEntriesChanged(entries: readonly StagedEntry[], omissions: Map<number, CommitOmission>): void {
  for (const entry of entries) {
    omissions.set(entry.index, commitOmission(entry.index, entry.path, "changed-during-load"))
  }
}

function recordOversizedCommitPaths(entries: readonly SizedEntry[], omissions: Map<number, CommitOmission>): void {
  for (const entry of entries) {
    omissions.set(
      entry.index,
...
async function captureCommitPatchParts(
  pi: ExtensionAPI,
  root: string,
  batches: readonly (readonly SizedEntry[])[],
  signal?: AbortSignal,
): Promise<string[]> {
  const parts: string[] = []
...
function retainedCommitCapture(
  parts: readonly string[],
  entries: readonly StagedEntry[],
  capturable: readonly SizedEntry[],
  omissions: Map<number, CommitOmission>,
  budget: CommitPromptBudget,
): CapturedCommitPatch {
...
async function captureSelectedCommitPatch(
  pi: ExtensionAPI,
  root: string,
  stagedRaw: string,
  entries: readonly StagedEntry[],
  ...
  budget: CommitPromptBudget,
  signal?: AbortSignal,
): Promise<CapturedCommitPatch> {
  if (selected.length === 0) return EMPTY_COMMIT_PATCH
...
export async function collectCommitDiffInput(
  pi: ExtensionAPI,
  cwd: string,
  budget: CommitPromptBudget = DEFAULT_COMMIT_PROMPT_BUDGET,
  signal?: AbortSignal,
): Promise<CommitDiffInput> {
  const root = await ensureGitRepository(pi, cwd, signal)
  if (!root) throw new Error("Not a git repository")
  const staged = await loadStagedEntries(pi, root, signal)
  if (staged.entries.length === 0) throw new Error("No staged changes to summarize")
  ...
  const captured = await captureSelectedCommitPatch(
    pi,
    root,
    staged.raw,
    staged.entries,
