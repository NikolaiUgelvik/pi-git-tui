...
export type SnapshotHeadState =
  | { kind: "initial"; branch: string }
  | { kind: "attached"; oid: string; branch: string }
...
export interface StatusEntry {
  ...
  submodule: string
  similarity?: { kind: "rename" | "copy"; score: number }
}
...
function parseFixedFields(record: string, marker: string, fieldCount: number): string[] {
...
function parseStatuses(value: string, record: string): [string, string] {
...
function validateObjectFields(record: string, modes: string[], oids: string[]): void {
...
function validateSubmodule(value: string, record: string): void {
  if (!/^(?:N\.\.\.|S[.C][.M][.U])$/u.test(value)) {
    throw new GitStatusParseError("Malformed porcelain-v2 submodule state", record)
  }
}

function objectExists(mode: string, oid: string): boolean {
  return mode !== "000000" && !/^0+$/u.test(oid)
}

function recordIndexIdentity(builder: SnapshotBuilder, values: readonly string[]): void {
  builder.indexIdentityParts.push(values.join("\0"))
}

function recordEntry(builder: SnapshotBuilder, entry: StatusEntry): void {
...
function parseOrdinaryRecord(builder: SnapshotBuilder, record: string): void {
  const [
    xy = "",
    submodule = "",
    headMode = "",
...
function parseSimilarity(value: string, record: string): StatusEntry["similarity"] {
...
function parseRenameRecord(builder: SnapshotBuilder, record: string, originalPath: string | undefined): void {
  if (!originalPath) {
...
function parseUnmergedRecord(builder: SnapshotBuilder, record: string): void {
  const [
    xy = "",
    submodule = "",
    baseMode = "",
...
function parseHeader(headers: StatusHeaders, record: string): void {
  const values: Array<[string, (value: string) => void]> = [
...
function parseHead(headers: StatusHeaders): SnapshotHeadState {
...
export function parsePorcelainV2(raw: string): WorkingTreeSnapshot {
  ...
  for (let index = 0; index < records.length; index++) {
    ...
    if (record.startsWith("# ")) {
      ...
      if (!path) throw new GitStatusParseError("Missing untracked path", record)
      builder.untrackedPaths.push(path)
    } else if (record.startsWith("! ")) {
      if (!record.slice(2)) throw new GitStatusParseError("Missing ignored path", record)
    } else {
...
export function workingTreeBranchLabel(snapshot: WorkingTreeSnapshot): string {
...
function branchFromSymbolicRef(ref: string): string {
  return ref.startsWith("refs/heads/") ? ref.slice("refs/heads/".length) : ref
}

function replaceSnapshotHead(snapshot: WorkingTreeSnapshot, head: SnapshotHeadState): WorkingTreeSnapshot {
  const updated = { ...snapshot, head }
  return { ...updated, statusFingerprint: workingTreeStatusFingerprint(updated) }
}

async function validateInitialHead(
  pi: ExtensionAPI,
  root: string,
...
async function disambiguateDetachedHead(
  pi: ExtensionAPI,
  root: string,
  snapshot: WorkingTreeSnapshot,
  signal?: AbortSignal,
