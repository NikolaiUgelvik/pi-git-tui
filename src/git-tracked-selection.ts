...
function indexSourceBytes(
  ...
  return measurementFromSizes(
    group.paths.map((path) => Math.max(headSizes.get(path) ?? 0, indexSizes.sizes.get(path) ?? 0)),
  )
}
...
function sourceBytes(
  ...
  scope: "combined" | "staged" | "working",
): SourceMeasurement {
  if (scope === "staged" && indexSizes) return indexSourceBytes(group, headSizes, indexSizes)
