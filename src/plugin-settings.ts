...
export function copyPluginSettings(settings: PluginSettings): PluginSettings {
  return { diff: { wrap: settings.diff.wrap } }
}

function decodedSettings(value: unknown): PluginSettings | undefined {
  if (!value || typeof value !== "object" || Array.isArray(value)) return
  const diff = (value as { diff?: unknown }).diff
  if (!diff || typeof diff !== "object" || Array.isArray(diff)) return
  const wrap = (diff as { wrap?: unknown }).wrap
...
export function createPluginSettingsStore(agentDirectory = getAgentDir()): PluginSettingsStore {
  ...
  return {
    ...
    async load() {
      ...
      try {
        ...
        if (errorCode(error) === "ENOENT") return { settings: copyPluginSettings(DEFAULT_PLUGIN_SETTINGS) }
        return {
