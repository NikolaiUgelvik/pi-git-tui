...
export interface ParsedTrustedText {
  ...
  readonly runs: { readonly start: number; readonly end: number; readonly state: SgrState }[]
}

export function emptySgrState(): SgrState {
  return {
    bold: false,
    faint: false,
    italic: false,
...
function cloneState(state: SgrState): SgrState {
  return { ...state }
}
...
function extendedColor(values: readonly number[], index: number): { value?: string; consumed: number } {
...
function applySgrAttribute(state: SgrState, value: number): boolean {
  switch (value) {
    case 0:
      resetState(state)
      break
...
function applySgrColor(state: SgrState, values: readonly number[], index: number): number {
  const value = values[index] ?? 0
  if ((value >= 30 && value <= 37) || (value >= 90 && value <= 97)) state.foreground = String(value)
  else if ((value >= 40 && value <= 47) || (value >= 100 && value <= 107)) state.background = String(value)
  else if (value === 39) delete state.foreground
  else if (value === 49) delete state.background
  else if (value === 38 || value === 48) {
    const color = extendedColor(values, index)
    if (color.value) {
      if (value === 38) state.foreground = color.value
...
function applySgrValues(state: SgrState, values: readonly number[]): void {
...
function readSgrCode(text: string, index: number): { length: number; values: number[] } | undefined {
  if (text[index] !== ESCAPE || text[index + 1] !== "[") return
  let end = index + 2
  while (end < text.length && /[0-9;]/u.test(text[end] ?? "")) end++
  if (text[end] !== "m") return
  const parameters = text.slice(index + 2, end)
  const rawValues = parameters === "" ? ["0"] : parameters.split(";")
  if (rawValues.some((value) => value !== "" && !/^\d+$/u.test(value))) return
  return {
    length: end + 1 - index,
...
export function canonicalSgrPrefix(state: SgrState): string {
  const values: string[] = []
  if (state.bold) values.push("1")
  if (state.faint) values.push("2")
  if (state.italic) values.push("3")
  if (state.underline) values.push("4")
  if (state.inverse) values.push("7")
  if (state.strikethrough) values.push("9")
  if (state.foreground) values.push(state.foreground)
  if (state.background) values.push(state.background)
  return values.length === 0 ? "" : `${ESCAPE}[${values.join(";")}m`
}

export function sgrStateFromAnsi(ansi: string | undefined): SgrState | undefined {
  if (ansi === undefined) return
...
function indexedRgb(index: number): RgbColor | undefined {
...
function sgrRgb(value: string | undefined): RgbColor | undefined {
  if (!value) return
  const parts = value.split(";").map(Number)
  if (parts.length === 5 && parts[1] === 2 && parts.slice(2).every((part) => Number.isInteger(part))) {
    return { red: parts[2] as number, green: parts[3] as number, blue: parts[4] as number }
  }
  if (parts.length === 3 && parts[1] === 5) return indexedRgb(parts[2] as number)
  const code = parts.length === 1 ? parts[0] : undefined
  if (code === undefined) return
  if (code >= 30 && code <= 37) return BASIC_COLORS[code - 30]
  if (code >= 90 && code <= 97) return BASIC_COLORS[code - 90 + 8]
...
function mixRgb(base: RgbColor, accent: RgbColor, accentWeight: number): RgbColor {
  const mix = (baseValue: number, accentValue: number): number =>
...
function closestIndexedColor(color: RgbColor): number {
  let closest = 16
  let distance = Number.POSITIVE_INFINITY
  for (let index = 16; index <= 255; index++) {
    const candidate = indexedRgb(index) as RgbColor
    const nextDistance =
      (candidate.red - color.red) ** 2 + (candidate.green - color.green) ** 2 + (candidate.blue - color.blue) ** 2
...
function isTrueColor(value: string | undefined): boolean {
  return value?.startsWith("38;2;") === true || value?.startsWith("48;2;") === true
}

export function blendedBackgroundAnsi(baseAnsi: string, accentAnsi: string, accentWeight = 0.3): string {
  ...
  if (!base || !accent) return baseAnsi
...
export function parseTrustedSgrText(text: string): ParsedTrustedText | undefined {
  const normalizedTabs = text.replace(/\t/gu, TAB_SPACES)
  const state = emptySgrState()
  const runs: { start: number; end: number; state: SgrState }[] = []
  let plainText = ""
  let index = 0
  while (index < normalizedTabs.length) {
    ...
    if (normalizedTabs.charCodeAt(index) === 0x9b) return
    ...
    plainText += chunk
    if (chunk.length > 0) runs.push({ start, end: plainText.length, state: cloneState(state) })
    index = end
  }
