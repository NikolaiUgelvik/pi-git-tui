...
export function confirmationDecision(data: string, allowLegacyQ = true): ConfirmationDecision {
...
export function confirmationHint(prompt: ConfirmationPrompt): string {
  return `Enter: ${prompt.confirmLabel} • Esc: Cancel • F1: Help`
}

export function initializationConfirmationPrompt(path: string): ConfirmationPrompt {
  return {
...
export function discardConfirmationPrompt(file: DiffFile | undefined): ConfirmationPrompt {
  if (!file) {
...
function trackedDiscardPrompt(path: string): ConfirmationPrompt {
  return {
...
function confirmationRows(text: string, width: number | undefined): string[] {
  if (width === undefined) {
    return [text]
  }
...
export function confirmationBodyLines(
  prompt: ConfirmationPrompt,
  theme: Theme,
  options: ConfirmationBodyOptions = {},
): string[] {
