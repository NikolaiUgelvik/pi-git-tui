...
function cleanSnapshotProbeArgs(snapshot: WorkingTreeSnapshot): string[] {
  return snapshot.head.kind === "initial"
    ? ["diff", "--cached", "--quiet", "--"]
    : ["diff", "--quiet", snapshot.head.oid, "--"]
}

function workingTreeDiffArgs(snapshot: WorkingTreeSnapshot, paths?: readonly string[]): string[] {
...
async function initialIndexMatches(
  pi: ExtensionAPI,
  root: string,
  ...
  if (!phase.initial || !phase.indexSizes) return true
...
function completeTrackedCapture(
  ...
  const changed = phase.initial ? new Set<number>() : changedGroups(patch.capturable, phase.before, patch.after)
