...
function splitWidths(innerWidth: number): { treeWidth: number; diffWidth: number } {
...
export function measureViewerGeometry(input: ViewerGeometryInput): ViewerGeometry {
  const width = whole(input.width)
  const height = Math.max(0, whole(input.terminalRows) - 2)
  const innerWidth = Math.max(0, width - 2)
  const density: ViewerDensity = width < 12 || height < 6 ? "too-small" : height < 9 ? "compact" : "normal"
  const layout: MainLayout =
    density === "too-small" ? "too-small" : input.empty ? "empty" : width >= SPLIT_LAYOUT_MIN_WIDTH ? "split" : "single"
...
export function measureOverlayGeometry(
  ...
  const density: OverlayDensity = availableWidth < 54 || availableHeight < 12 ? "compact" : "normal"
